...
class Models:
    ...
    def get_model(self, args, overal_maxlen, vocab):
        ...
        if args.model_type == 'cyh':
            from my_layers import HSMMBottom, HSMMTower, Self_Attention
            from tensorflow.keras import Model
            from tensorflow.keras.layers import Input, Dense, Embedding, GlobalAveragePooling1D, GlobalMaxPooling1D, \
                Concatenate, Convolution1D, Dropout, MaxPool1D \
                , LSTM, LeakyReLU, concatenate, Flatten
            ...
            # emb_out1.shape == (None,600,300)
            emb_output = Embedding(args.vocab_size, 50, name='emb')(input1)
            mul_out = MultiHeadAttention(5,10)(emb_output)
            max = GlobalMaxPooling1D()(mul_out)
            avg = GlobalAveragePooling1D()(mul_out)
            mul_out = concatenate([avg, max], axis=-1)
            ...
            bert_model1.summary()
            dim_out1 = bert_model1(
                {"input_ids": input_ids, "token_type_ids": input_tokentype, "attention_mask": input_mask})
            bert_1 = dim_out1.last_hidden_state
            max = GlobalMaxPooling1D()(bert_1)
            avg = GlobalAveragePooling1D()(bert_1)
            bert_1 = concatenate([avg, max], axis=-1)
            bert_1 = Dropout(0.2)(bert_1)
            fully_connected_layers = [200, 200]
            for fl in fully_connected_layers:
                bert_1 = Dense(fl, activation='relu')(bert_1)
            ...
            bert_2 = concatenate([avg, max], axis=-1)
            bert_2 = Dropout(0.2)(bert_2)
            fully_connected_layers = [200, 200]
            for fl in fully_connected_layers:
                bert_2 = Dense(fl, activation='relu')(bert_2)

            x = concatenate([mul_out,bert_1,bert_2], axis=-1)
            x = Dropout(0.2)(x)
            x = Dense(100, activation='swish')(x)
            x = Dense(100, activation='swish')(x)
            x = Dropout(0.1)(x)
